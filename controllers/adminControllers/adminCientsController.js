...
const reActive = async (req,res) => {
    const { _id } = req.body;
    try{
        const client = await clientModel.updateIsLocked(_id,false)
...
const getClientsRequests = async (req,res) => {
    try{
        const resuests = await reqModel.getClientsRequests()

        res.status(200).json(resuests)
    } catch(err) {
        res.status(400).json({message:err.message})
    }
}
...
const deleteClientRequest = async (req,res) => {
    ...
    try{
        const client = await reqModel.findOneAndDelete({_id})
